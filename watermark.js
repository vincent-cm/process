...
function _isValidAlignment(align) {
  ...
  //
  if (
    ["dia1", "dia2", "ttb", "btt", "ltr", "rtl"].indexOf(align.toLowerCase()) >
    -1
  )
    return true;
...
function _isValidPosition(position) {
  if (ratify.isEmpty(position)) return false;
...
function _parseOptions(imageData, source, options) {
  ...
  var align = _isValidAlignment(options.align)
    ? options.align.toLowerCase()
    : "dia1";
  var font = options.font;
  var resize = options.resize ? options.resize : defaultOptions.resize;
  var outputPath = options.dstPath
    ? options.dstPath
    : path.dirname(source) + "/watermark" + path.extname(source);
  var position = _isValidPosition(options.position)
    ? options.position
    : "Center";
  ...
  if (ratify.isEmpty(fillColor)) fillColor = defaultOptions.color;

  // Check if watermark text is specified
  if (ratify.isEmpty(watermarkText)) watermarkText = defaultOptions.text;

  // Check if position is specified
  if (ratify.isEmpty(position)) position = defaultOptions.position;

  // Check if weight is specified
  if (ratify.isEmpty(weight)) weight = defaultOptions.weight;

  // Check if offset is specified
  if (ratify.isEmpty(offset)) offset = defaultOptions.offset;

  // Check if image needs to be overriden
  if (
    options["override-image"] &&
    ratify.isBoolean(options["override-image"]) &&
  ...
  if (
    ...
    ratify.isBoolean(options["change-format"]) &&
  ...
  switch (align) {
    case "ltr":
      ...
      pointsize = options.pointsize
        ? options.pointsize
        : pointWidth / strLen(watermarkText);
      break;
    case "rtl":
      angle = 180;
      pointsize = options.pointsize
    ...
    case "ttb":
      ...
      pointsize = options.pointsize
        ? options.pointsize
        : pointHeight / strLen(watermarkText);
      break;
    case "btt":
      angle = 270;
      pointsize = options.pointsize
    ...
    case "dia1":
      ...
      pointsize = options.pointsize
        ? options.pointsize
        : Math.sqrt(pointWidth * pointWidth + pointHeight * pointHeight) /
        strLen(watermarkText);
      break;
    case "dia2":
  ...
  args.push("-annotate");
  args.push(angle + "x0" + offset); // angle of watermark message, with respect to X-axis
...
function embedWatermark(source, options) {
  if (!source || source == "")
    throw new Error(
      "Image-Watermark::embedWatermark : Specified an invalid image source"
    );
  ...
  if (!stats.isFile())
    throw new Error(
      "Image-Watermark::embedWatermark : Image does not exists at : " + source
    );
  ...
  im.identify(source, function (err, imageData) {
    if (err)
      throw new Error(
        "Image-Watermark::embedWatermark : Unable to process image file : " +
          err
      );
    ...
    im.convert(retObj.args, function (err, stdout) {
      if (err)
        console.log(
          "Image-Watermark::embedWatermark : Error in applying watermark : " +
            err
        );
      else
        console.log(
          "Image-Watermark::embedWatermark : Successfully applied watermark. Please check it at :\n " +
            retObj.outputPath
        );
    });
  });
...
function embedWatermarkWithCb(source, options, callback) {
  ...
  if (
    ...
    throw new Error(
      "Image-Watermark::embedWatermarkWithCb : Invalid arguments to method embedWatermarkWithCb"
    );
  ...
  if (!source || source == "") {
    error = new Error(
      "Image-Watermark::embedWatermarkWithCb : Specified an invalid image source"
    );
  ...
  fs.lstat(source, function (err, stats) {
    if (err || !stats.isFile()) {
      error = new Error(
        "Image-Watermark::embedWatermarkWithCb : Image file doesn't exists at " +
          source
      );
      return callback(error);
    } else if (!err) {
      ...
      im.identify(source, function (err, imageData) {
        if (err) {
          error = new Error(
            "Image-Watermark::embedWatermarkWithCb : Unable to process image file : " +
              err
          );
          return callback(error);
        }
        ...
        im.convert(retObj.args, function (err, stdout) {
          if (err) {
            error = new Error(
              "Image-Watermark::embedWatermarkWithCb : Error in applying watermark : " +
                err
            );
            return callback(error);
          } else {
            console.log(
              "Image-Watermark::embedWatermarkWithCb : Successfully applied watermark. Please check it at :\n " +
                retObj.outputPath
            );
